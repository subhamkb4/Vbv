...
# Your VBV lookup function with improved error handling
def vbv_lookup(card_number, exp_month, exp_year, cvv):
    ...
    try:
        ...
        if wiz1 == -1:
            return {
                'card': f"{card_number}|{exp_month}|{exp_year}|{cvv}",
                'status': "ERROR: Token end marker not found",
                'elapsed_time': time.time() - start_time,
                'success': False,
                'bin': card_number[:6]
            }
        ...
        # Improved base64 decoding with multiple encoding attempts
        try:
            ...
            try:
                ...
                decode_text = base64.urlsafe_b64decode(encoded_text).decode('utf-8')
            except:
                try:
                    ...
                    return {
                        'card': f"{card_number}|{exp_month}|{exp_year}|{cvv}",
                        'status': f"ERROR: Base64 decode failed - {str(decode_error)}",
                        'elapsed_time': time.time() - start_time,
                        'success': False,
                        'bin': card_number[:6]
                    }
        ...
        wizard_matches = re.findall('"authorizationFingerprint":"(.*?)"', decode_text)
        if not wizard_matches:
            return {
                'card': f"{card_number}|{exp_month}|{exp_year}|{cvv}",
                'status': "ERROR: Authorization fingerprint not found",
                'elapsed_time': time.time() - start_time,
                'success': False,
                'bin': card_number[:6]
            }
        
        wizard = wizard_matches[0]
        ...
        if response.status_code != 200:
            return {
                'card': f"{card_number}|{exp_month}|{exp_year}|{cvv}",
                'status': f"ERROR: Tokenization failed with status {response.status_code}",
                'elapsed_time': time.time() - start_time,
                'success': False,
                'bin': card_number[:6]
            }
        ...
        if 'data' not in token_data or 'tokenizeCreditCard' not in token_data['data']:
            return {
                'card': f"{card_number}|{exp_month}|{exp_year}|{cvv}",
                'status': "ERROR: Tokenization response invalid",
                'elapsed_time': time.time() - start_time,
                'success': False,
                'bin': card_number[:6]
            }

        wizard1 = token_data['data']['tokenizeCreditCard']['token']

        # Step 3: 3D Secure Lookup
        headers = {
...
def home():
    return jsonify({
        ...
        "example": "https://your-app.onrender.com/gateway?key=blazedev&card=4111111111111111|12|25|123"
    })
